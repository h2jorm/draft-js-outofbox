  ...
  if (!data.isRichText()) {
    ...
    if (files.length > 0) {
      ...
      getTextContentFromFiles(files, function ( /*string*/fileText) {
        ...
        var editorState = _this._latestEditorState;
  ...
  if (!this.props.stripPastedStyles) {
    ...
    if (data.isRichText() && internalClipboard) {
      ...
      textBlocks.length === 1 && internalClipboard.size === 1 && internalClipboard.first().getText() === text) {
        this.update(insertFragment(this._latestEditorState, internalClipboard));
        return;
      }
    ...
    if (html) {
      ...
      if (htmlFragment) {
        ...
        if (contentBlocks) {
          ...
          this.update(insertFragment(this._latestEditorState, htmlMap, entityMap));
          return;
        }
      }
    }
